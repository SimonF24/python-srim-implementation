...
class SRIM_Output(object):
    ...
    def _read_ion(self, output):
        ion_regex = fr'Ion\s+=\s+({symbol_regex})\s+Energy\s+=\s+({double_regex})\s+keV'
    ...
    def _read_table(self, output):
        match = re.search(bytes(fr'^{float_regex}.*{float_regex}', 'utf-8'), output, re.DOTALL | re.MULTILINE)
        ...
        if match:
            ...
            # Data
            data = np.genfromtxt(BytesIO(match.group(0)))
            return data
        raise SRIMOutputParseError("unable to extract table from file")
    
    def _read_alt_table(self, output):
...
class Results(object):
    ...
    Files that are looked for:
      ...
      - ``BACKSCAT.txt`` handled by :class:`srim.output.Backscat`
      - ``SPUTTER.txt`` handled by :class:`srim.output.Sputter`
      - ``TRANSMIT.txt`` handled by :class:`srim.output.Transmit`
      - ``TRIMOUT.txt`` handled by :class:`srim.output.Trimout`
    """
    def __init__(self, directory):
        ...
        try:
            self.backscat = Backscat(directory)
        except FileNotFoundError:
            try:
                self.backscat = Backscat(directory + '/SRIM Outputs')
            except:
                self.backscat = None
        except SRIMOutputParseError:
            self.backscat = None
        try:
            self.sputter = Sputter(directory)
        except FileNotFoundError:
            try:
                self.sputter = Sputter(directory + '/SRIM Outputs')
            except:
                self.sputter = None
        except SRIMOutputParseError:
            self.sputter = None
        try:
            self.transmit = Transmit(directory)
        except FileNotFoundError:
            try:
                self.transmit = Transmit(directory + '/SRIM Outputs')
            except:
                self.transmit = None
        except SRIMOutputParseError:
            self.transmit = None
        try:
            self.trimout = Trimout(directory)
        except FileNotFoundError:
            try:
                self.trimout = Trimout(directory + '/SRIM Outputs')
            except:
                self.trimout = None
        except SRIMOutputParseError:
            self.trimout = None
...
class Backscat(SRIM_Output):
    ...
    directory : :obj:`str`
         directory of calculation
    filename : :obj:`str`, optional
         filename for Backscat. Default ``BACKSCAT.txt``
    """
    def __init__(self, directory, filename='BACKSCAT.txt'):
        with open(os.path.join(directory, filename), 'rb') as f:
            output = f.read()
            ion = self._read_alt_ion(output)
            data = self._read_alt_table(output)

        self._ion = ion
        self._interaction_type = data[:, 0]
        self._ion_num = data[:, 1].astype(int)
        self._atom_num = data[:, 2].astype(int)
        self._energy = data[:, 3].astype(float)
        self._depth = data[:, 4].astype(float)
        self._lateral_pos_y = data[:, 5].astype(float)
        self._lateral_pos_z = data[:, 6].astype(float)
        self._atom_dir_x = data[:, 7].astype(float)
        self._atom_dir_y = data[:, 8].astype(float)
        self._atom_dir_z = data[:, 9].astype(float)
    ...
    def interaction_type(self):
        """Type of interaction experienced by the ion or atom
    ...
    @property
    def ion_num(self):
        """Sequential number of the ion in the calculation
        """
        return self._ion_num
    
    @property
    def atom_num(self):
        """Atomic number of the baskscattered ion
        """
        return self._atom_num

    @property
    def energy(self):
        """Energy (eV) of the backscattered ion as it leaves the target
        """
        return self._energy

    @property
    def depth(self):
        """Depth [Ang] of the ion when it was declared backscattered

        Since target atoms are randomly placed within their lattice cells, this 
        will vary slightly from zero, but will always be in the upper monolayer 
        of the target
        """
        return self._energy

    @property
    def lateral_pos_y(self):
        """The lateral position [Ang] of the ion in the y-direction 
        when it exits the target
        """
        return self._lateral_pos_y

    @property
    def lateral_pos_z(self):
        """The lateral position [Ang] of the ion in the z-direction
        when it exits the target
        """
        return self._lateral_pos_z

    @property
    def atom_dir_x(self):
        """The x-directional cosine of the trajectory of the backscattered ion

        This will always be negative since the x-axis measures depth into the 
        target and this indicates a backwards trajectory
        """
        return self._atom_dir_x

    @property
    def atom_dir_y(self):
        """The y-directional cosine of the trajectory of the backscattered ion
        """
        return self._atom_dir_y

    @property
    def atom_dir_z(self):
        """The z-directional cosine of the trajectory of the backscattered ion
        """
        return self._atom_dir_z


class Transmit(SRIM_Output):
    ...
    directory : :obj:`str`
         directory of calculation
    filename : :obj:`str`, optional
         filename for Transmit. Default ``TRANSMIT.txt``
    """
    ...
    @property
    def atom_num(self):
        '''Atomic number of the transmitted ion
        '''
        return self._atom_num

    @property
    def energy(self):
        '''Energy (eV) of the transmitted ion 
        '''
        return self._energy

    @property
    def depth(self):
        '''Depth [Ang] of the ion when it was declared transmitted

        Since target atoms are randomly placed within their lattice cells, this
        will vary slightly from the total target depth
        '''
        return self._depth

    @property
    def lateral_pos_y(self):
        '''The lateral position [Ang] of the ion when in the y-direction
        it exits the target
        '''
        return self._lateral_pos_y
    ...
    def lateral_pos_z(self):
        ...
        when it exits the target
        '''
        return self.lateral_pos_z

    @property
    def atom_dir_x(self):
        '''The x-directional cosine of the trajectory of the transmitted ion
        '''
        return self._atom_dir_x

    @property
    def atom_dir_y(self):
        '''The y-directional cosine of the trajectory of the transmitted ion
        '''
        return self._atom_dir_z

    @property
    def atom_dir_z(self):
        '''The z-directional cosine of the trajectory of the transmitted ion
        '''


class Sputter(SRIM_Output):
    ...
    directory : :obj:`str`
         directory of calculation
    filename : :obj:`str`, optional
         filename for Sputter. Default ``SPUTTER.txt``
    """
    def __init__(self, directory, filename='SPUTTER.txt'):
        with open(os.path.join(directory, filename), 'rb') as f:
            output = f.read()
            ion = self._read_alt_ion(output)
            data = self._read_alt_table(output)
    ...
    @property
    def ion_num(self):
        '''Sequential number of the ion that caused the atom to sputter
        in the calculation
        '''
        return self._ion_num
    ...
    @property
    def atom_num(self):
        '''Atomic number of the sputtered atom
        '''
        return self._atom_num

    @property
    def energy(self):
        '''Energy (eV) of the sputtered atom
        '''
        return self._energy

    @property
    def depth(self):
        '''Depth [Ang] of the atom when it was declared sputtered
        '''
        return self._depth
    
    @property
    def lateral_pos_y(self):
        '''The lateral position [Ang] of the atom in the y-direction
        when it exits the target
        '''
        return self._lateral_pos_y
    ...
    @property
    def lateral_pos_z(self):
        '''The lateral position [Ang] of the atom in the z-direction
        when it exits the target
        '''
        return self._lateral_pos_z
    ...
    @property
    def atom_dir_x(self):
        '''The x-directional cosine of the sputtered atom
    ...
    @property
    def atom_dir_y(self):
        '''The y-directional cosine of the sputtered atom
        '''
        return self._atom_dir_y
    
    @property
    def atom_dir_z(self):
        '''The z-directional cosine of the sputtered atom
        '''
        return self._atom_dir_z


class Trimout(SRIM_Output):
    """ The kinetics of all backscattered ions, transmitted ions, or sputtered
    atoms
    ...
    directory : :obj:`str`
         directory of calculation
    filename : :obj:`str`, optional
         filename for Trimout. Default ``TRIMOUT.txt``
    """
    def __init__(self, directory, filename='TRIMOUT.txt'):
        with open(os.path.join(directory, filename), 'rb') as f:
            output = f.read()
            ion = self._read_alt_ion(output)
            data = self._read_alt_table(output)
    ...
    @property
    def ion_num(self):
        '''Sequential number of the ion that caused the interaction in the
        calculation
        '''
        return self._ion_num
    ...
    @property
    def atom_num(self):
        '''Atomic number of the ion or atom
        '''
        return self._atom_num

    @property
    def energy(self):
        '''Energy (eV) of the ion or atom
        '''
        return self._energy

    @property
    def depth(self):
        '''Depth [Ang] of the atom when it was declared backscattered,
        transmitted, or sputtered
        '''
        return self._depth
    
    @property
    def lateral_pos_y(self):
        '''The lateral position [Ang] of the ion or atom in the y-direction
        when it exits the target
        '''
        return self._lateral_pos_y
    ...
    @property
    def lateral_pos_z(self):
        '''The lateral position [Ang] of the ion or atom in the z-direction
        when it exits the target
        '''
        return self._lateral_pos_z
    ...
    @property
    def atom_dir_x(self):
        '''The x-directional cosine of the ion or atom
    ...
    @property
    def atom_dir_y(self):
        '''The y-directional cosine of the ion or atom
        '''
        return self._atom_dir_y
    
    @property
    def atom_dir_z(self):
        '''The z-directional cosine of the ion or atom
        '''
        return self._atom_dir_z
